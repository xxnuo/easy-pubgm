...
try {
    ...
    // 直接全局安装, pnpm 会根据版本判断是否需要更新
    execSync('pnpm install -g @sm003/autojs6-dts', { stdio: 'inherit' });
} catch (err) {
...
try {
    ...
    globalNodeModules = execSync('pnpm root -g', { encoding: 'utf8' }).trim();
} catch (err) {
    console.error('获取全局安装路径失败, 请检查 pnpm 配置', err);
    process.exit(1);
}
